...
bool DiskDatabase::connect(const char* db_file) {
  const char* tmp= "PRAGMA foreign_keys = ON";
  bool res = sqlite3_open(db_file, &db) == SQLITE_OK;
  sqlite3_exec(db, tmp, NULL, 0, NULL);
  return res;
}

vector<pair<int, string>> DiskDatabase::get_table_data(unsigned int grpID) {
  sqlite3_stmt *statement;
  vector<pair<int, string>> pairs;
  bool ng = false;
  char *query = (char*)"SELECT * FROM articles WHERE grpID = ?";
  if(grpID == -1){
    query =(char*)"SELECT * FROM newsgroups";
    ng = true;
  }
  ...
  if (sqlite3_prepare(db, query, -1, &statement, 0) == SQLITE_OK) {
    ...
    while (1) {
      ...
      if (res == SQLITE_ROW) {
        pairs.push_back(std::make_pair(sqlite3_column_int(statement, 0),(char*)sqlite3_column_text(statement, 1)));
      }
...
void DiskDatabase::OLD_get_table_data() {
  sqlite3_stmt *statement;
...
bool DiskDatabase::insert_news_group(string title) {
  sqlite3_stmt *statement;
...
bool DiskDatabase::insert_article(unsigned int grpID, string title, string author, string text) {
  sqlite3_stmt *statement;
...
bool DiskDatabase::has_ng(unsigned int grpID){
  sqlite3_stmt *statement;
...
bool DiskDatabase::has_art(unsigned int grpID, unsigned int artID) {
  sqlite3_stmt *statement;
...
Article DiskDatabase::get_art(unsigned int grpID, unsigned int artID) {
  sqlite3_stmt *statement;
  int art_i, grp_i;
...
void DiskDatabase::createDB(const char* db_file) {
  ...
  //
	// pSQL[3] = "INSERT \
  //           INTO articles (title, author, txt, grpID) \
...
DiskDatabase::DiskDatabase() {
  ...
  // get_articles(-1); // Just to test functions
  // get_articles(3);
}
...
vector<pair<int, string>> DiskDatabase::list_news_groups() {
  return get_table_data(-1);

}
pair<vector<pair<int, string>>, bool> DiskDatabase::list_articles(int grpID) {
  if(!has_ng(grpID)){
    return std::make_pair(get_table_data(grpID), false);
  }
  return std::make_pair(get_table_data(grpID), true);
}
bool DiskDatabase::create_news_group(string name) {
  return insert_news_group(name);
}
bool DiskDatabase::create_article(int grpID, string name, string author, string text) {
  return insert_article(grpID,name,author,text);
}
...
std::pair<Article, int> DiskDatabase::get_article(int grpID, int artID) {
  Article art;
  if(!has_ng(grpID)){
    return std::make_pair(art,3);
  }
  if(!has_art(grpID,artID)){
    return std::make_pair(art,2);
  }
  art = get_art(grpID,artID);
  return std::make_pair(art,1);
}
